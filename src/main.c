...
loaded:
        ...
        if (cet_posture && e.format == 1) {
            ...
            uint32_t dx = e.pe_dll_chars_ex;
            fprintf(stdout, "# cet-posture %s (pe):", path);
            fprintf(stdout,
                " ASLR=%s CFG=%s DEP=%s HIGH_ENTROPY=%s CET_COMPAT=%s",
                ...
                (dx & IMAGE_DLLCHARACTERISTICS_EX_CET_COMPAT)  ? "on" : "off");
            if (e.pe_pdb_path[0])
                fprintf(stdout, " pdb=%s", e.pe_pdb_path);
            fputc('\n', stdout);
        } else if (cet_posture) {
